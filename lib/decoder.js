...
function Decoder (opts) {
  ...
  r = binding.ogg_sync_init(this.oy);
  if (0 !== r) {
...
Decoder.prototype._write = function (chunk, done) {
  ...
  function readPage (err) {
    debug('readPage(%j)', err);
    if (err) {
      done(err);
    } else {
      binding.ogg_sync_pageout(oy, page, afterPage);
    }
  }

  function afterPage (rtn, serialno) {
    debug('afterPage(%d, %d)', rtn, serialno);
    if (0 === rtn) {
      // need more data
      done();
    } else if (1 === rtn) {
      ...
      asyncEmit(stream, 'page', readPage);
    } else {
      // something bad...
...
Decoder.prototype._stream = function (serialno) {
  ...
  if (!stream) {
    ...
    this[serialno] = stream = new EventEmitter();
