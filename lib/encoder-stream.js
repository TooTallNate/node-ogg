...
/**
 * The `EncoderStream` class abstractions the `ogg_stream` data structure when
 * used with the encoding interface.
 *
 * @api public
 */
...
EncoderStream.prototype._pageout = function (fn) {
  ...
  binding.ogg_stream_pageout(os, og, function (rtn, hlen, blen, e_o_s) {
    debug('ogg_stream_pageout() return = %d (hlen=%d) (blen=%d) (eos=%d)', rtn, hlen, blen, e_o_s);
    if (0 === rtn) {
      fn();
    } else {
