...
NAN_METHOD(node_ogg_sync_write) {
  ...
  long size = static_cast<long>(info[2]->NumberValue(Nan::GetCurrentContext()).FromJust());
...
NAN_METHOD(node_ogg_stream_init) {
  ...
  int serialno = static_cast<int>(info[1]->IntegerValue(Nan::GetCurrentContext()).FromJust());
